...
func (b *Box) MouseHandler() func(action tview.MouseAction, event *tcell.EventMouse, setFocus func(p tview.Primitive)) (consumed bool, capture tview.Primitive) {
	return b.WrapMouseHandler(func(action tview.MouseAction, event *tcell.EventMouse, setFocus func(p tview.Primitive)) (consumed bool, capture tview.Primitive) {
		if action == tview.MouseLeftDown && b.InRect(event.Position()) {
			setFocus(b)
			consumed = true
		}
		return
...
func (b *Box) DrawForSubclass(screen tcell.Screen, p tview.Primitive) {
	...
	if b.border && b.width >= 2 && b.height >= 2 {
		...
		if b.title != "" && b.width >= 4 {
			printed, _ := tview.Print(screen, b.title, b.x+1, b.y, b.width-2, b.titleAlign, b.titleColor)
			if len(b.title)-printed > 0 && printed > 0 {
				xEllipsis := b.x + b.width - 2
				if b.titleAlign == tview.AlignRight {
					xEllipsis = b.x + 1
				}
				_, _, style, _ := screen.GetContent(xEllipsis, b.y)
				fg, _, _ := style.Decompose()
				tview.Print(screen, string(tview.SemigraphicsHorizontalEllipsis), xEllipsis, b.y, 1, tview.AlignLeft, fg)
			}
		}
	}
