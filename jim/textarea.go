...
func (t *TextArea) Draw(screen tcell.Screen) {
	...
	if t.labelWidth > 0 {
		...
		printWithStyle(screen, t.label, x, y, 0, labelWidth, tview.AlignLeft, t.labelStyle, labelBg == tcell.ColorDefault)
		x += labelWidth
		width -= labelWidth
	} else {
		_, _, drawnWidth := printWithStyle(screen, t.label, x, y, 0, width, tview.AlignLeft, t.labelStyle, labelBg == tcell.ColorDefault)
		x += drawnWidth
		width -= drawnWidth
	}
	...
	for pos[0] != 1 {
		...
		if string(runes) == "\n" {
			max := t.rowOffset + height
			if t.LineNumbers <= max {
				// draw current line number
				toStr := strconv.Itoa(t.rowOffset + t.LineNumbers + 1)
				if len(toStr) < 4 {
					for i := 0; i < 4-len(toStr); i++ {
						toStr = " " + toStr
					}
				}
				for i, c := range toStr {
					screen.SetContent(x-4+i, y+posY, c, nil, tcell.StyleDefault)
				}
			}
...
func (t *TextArea) MouseHandler() func(action tview.MouseAction, event *tcell.EventMouse, setFocus func(p tview.Primitive)) (consumed bool, capture tview.Primitive) {
	return t.WrapMouseHandler(func(action tview.MouseAction, event *tcell.EventMouse, setFocus func(p tview.Primitive)) (consumed bool, capture tview.Primitive) {
		...
		if labelWidth == 0 && t.label != "" {
			labelWidth = tview.TaggedStringWidth(t.label)
		}
		...
		case tview.MouseLeftUp:
			...
			capture = nil
			t.dragging = false
		case tview.MouseLeftDoubleClick: // Select word.
